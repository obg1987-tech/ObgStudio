...
const DynamicLogo = ({ genre }) => {
    ...
    if (genre === 'Rock') {
        return (
            <div className="flex items-center cursor-pointer relative theme-rock-jitter">
                {/* 락: 번개 아이콘 */}
                <svg className="w-8 h-8 md:w-10 md:h-10 text-[#ff4500] drop-shadow-[0_0_15px_#ff4500]" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M13 2L3 14H12L11 22L21 10H12L13 2Z" />
                </svg>
                {/* 날카로운 폰트 */}
                <span className="text-3xl md:text-5xl font-black italic tracking-[-0.08em] ml-2 text-white drop-shadow-[0_0_10px_rgba(255,69,0,0.8)] uppercase">
                    ObgStudio
                </span>
                {/* 데코: 불꽃 or 작은 번개 */}
                <svg className="w-5 h-5 md:w-7 md:h-7 text-yellow-400 ml-1 -translate-y-4 drop-shadow-[0_0_10px_#ffeb3b]" fill="currentColor" viewBox="0 0 24 24">
    ...
    if (genre === 'Lullaby') {
        return (
            <div className="flex items-center cursor-pointer relative theme-lofi-float">
                {/* 로파이: 달 아이콘 */}
                <svg className="w-8 h-8 md:w-10 md:h-10 text-[#81ecec] drop-shadow-[0_0_15px_#81ecec]" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 3a9 9 0 109 9c0-.46-.04-.92-.1-1.36a5.389 5.389 0 01-4.4 2.26 5.403 5.403 0 01-3.14-9.8c-.44-.06-.9-.1-1.36-.1z" />
                </svg>
                {/* 둥글고 부드러운 폰트 */}
                <span className="text-2xl md:text-4xl font-semibold tracking-wide ml-3 theme-lofi-text capitalize" style={{ fontFamily: 'Quicksand, Nunito, sans-serif' }}>
                    ObgStudio
                </span>
                {/* 데코: 구름/Zzz */}
                <svg className="w-6 h-6 md:w-8 md:h-8 text-[#a29bfe] ml-2 -translate-y-5 animate-[pulse_4s_infinite]" fill="currentColor" viewBox="0 0 24 24">
    ...
    if (genre === 'Hip-hop') {
        return (
            <div className="flex items-center cursor-pointer relative">
                ...
                {/* EDM: 바이닐/레코드판 아이콘 */}
                <svg className="w-8 h-8 md:w-10 md:h-10 text-white animate-[spin_2s_linear_infinite]" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 14.5c-2.49 0-4.5-2.01-4.5-4.5S9.51 7.5 12 7.5s4.5 2.01 4.5 4.5-2.01 4.5-4.5 4.5zm0-5.5c-.55 0-1 .45-1 1s.45 1 1 1 1-.45 1-1-.45-1-1-1z" />
                </svg>
                {/* 글리치 & 모노스페이스 폰트 */}
                <span className="text-2xl md:text-3xl font-black italic tracking-widest ml-4 theme-edm-glitch-text uppercase" style={{ fontFamily: 'monospace' }}>
                    ObgStudio
                </span>
                {/* 데코: 디지털 픽셀 십자 */}
                <svg className="w-4 h-4 md:w-6 md:h-6 text-[#ff00ff] ml-2 -translate-y-6" fill="currentColor" viewBox="0 0 24 24">
    ...
    if (genre === 'K-Pop') {
        return (
            <div className="flex items-center cursor-pointer relative transition-transform duration-300 hover:scale-105">
                {/* Kpop: 심장/하트 아이콘 비트 모션 */}
                <svg className="w-8 h-8 md:w-10 md:h-10 text-[#ff71ce] drop-shadow-[0_0_15px_#ff71ce] theme-kpop-heartbeat" fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
                </svg>
                {/* Kpop: 둥글고 귀여운 폰트에 멀티컬러 그라데이션 */}
                <span className="text-3xl md:text-5xl font-black tracking-tight ml-3 theme-kpop-text" style={{ fontFamily: '"Comic Sans MS", "Arial Rounded MT Bold", sans-serif' }}>
                    ObgStudio
                </span>
                {/* 데코: 스파클(Sparkles) 아이콘 */}
                <svg className="w-5 h-5 md:w-7 md:h-7 text-[#01cdfe] ml-1 -translate-y-5 animate-[spin_3s_linear_infinite_reverse] drop-shadow-[0_0_10px_#01cdfe]" fill="currentColor" viewBox="0 0 24 24">
    ...
    if (genre === 'Jazz') {
        return (
            <div className="flex items-center cursor-pointer relative transition-all duration-1000">
                <div className="theme-jazz-ornament"></div>
                {/* 재즈: 음표 아이콘 */}
                <svg className="w-8 h-8 md:w-10 md:h-10 drop-shadow-[0_0_8px_rgba(212,175,55,0.8)]" style={{ color: '#d4af37' }} fill="currentColor" viewBox="0 0 24 24">
                    <path d="M12 3v10.55c-.59-.34-1.27-.55-2-.55-2.21 0-4 1.79-4 4s1.79 4 4 4 4-1.79 4-4V7h4V3h-6z" />
                </svg>
                {/* 엘레강스 세리프 폰트 */}
                <span className="text-3xl md:text-5xl font-serif tracking-[0.05em] ml-2 theme-jazz-shimmer-text">
                    ObgStudio
                </span>
                {/* 데코: 다이아몬드 별 */}
                <svg className="w-4 h-4 md:w-5 md:h-5 text-white drop-shadow-[0_0_10px_white] ml-2 -translate-y-4 opacity-80" fill="currentColor" viewBox="0 0 24 24">
    ...
    return (
        <div className="flex items-center cursor-pointer logo-wrap hover:scale-105 transition-transform duration-500 relative">
            ...
            <div className="flex gap-2 transform -rotate-45 relative translate-y-[2px] logo-bars transition-all duration-500">
                <div className="w-[10px] h-[22px] md:w-[12px] md:h-[26px] rounded-full drop-shadow-md logo-bar transition-all duration-500"></div>
                <div className="w-[10px] h-[22px] md:w-[12px] md:h-[26px] rounded-full drop-shadow-md translate-y-[8px] md:translate-y-[10px] logo-bar transition-all duration-500"></div>
            </div>
            {/* 텍스트 폰트 및 별 모양 데코레이션 */}
            <span className="text-2xl md:text-4xl font-[800] tracking-[-0.03em] ml-3 logo-text transition-all duration-500">
                ObgStudio
            </span>
            <svg className="w-5 h-5 md:w-7 md:h-7 ml-[4px] -translate-y-[8px] md:-translate-y-[12px] logo-star transition-all duration-500" viewBox="0 0 24 24" fill="currentColor">
